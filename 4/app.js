...
function buildChannels() {
  ...
  for (let i = 0; i <= spanA; i++) {
    ...
    channels.push({
      id,
      name:  CHANNEL_NAMES_A[id] || 'Canal ' + globalNum,
      url:   `${BASE_URL}${id}${BASE_PATH}`,
      num:   String(globalNum).padStart(2, '0'),
      group: 'A',
    });
    globalNum++;
  }
  ...
  for (let i = 0; i <= spanB; i++) {
    ...
    channels.push({
      id,
      name:  CHANNEL_NAMES_B[id] || 'Canal ' + globalNum,
      url:   `${BASE_URL}${id}${BASE_PATH}`,
      num:   String(globalNum).padStart(2, '0'),
      group: 'B',
    });
...
function renderGrid(channels) {
  ...
  channels.forEach((ch, i) => {
    ...
    card.style.setProperty('--dot-delay', `${(i * 0.12) % 2}s`);
    ...
    card.addEventListener('click', () => openPlayer(ch.url, ch.name));
...
function openPlayer(url, name) {
  ...
  if (Hls.isSupported()) {
    ...
    overlay.innerHTML = `<p style="color:#c8622a;font-family:var(--ff-mono);font-size:0.72rem;letter-spacing:0.25em;text-transform:uppercase">HLS no soportado</p>`;
  }
}
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && !document.getElementById('playerModal').hidden) closePlayer();
});
...
function initTicker() {
  ...
  const original = Array.from(inner.children);
...
function initHeat() {
  ...
  function newMote(atBottom = false) {
    return {
      x:       Math.random() * W,
  ...
  spawnColumns();
  requestAnimationFrame(drawFrame);
  window.addEventListener('resize', () => { resize(); spawnMotes(); spawnColumns(); });
}

document.addEventListener('DOMContentLoaded', () => {
  initHeat();
  initTicker();
  renderGrid(buildChannels());
});
